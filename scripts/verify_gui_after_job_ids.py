...
BAD_PATTERNS = [
    ...
    (re.compile(r"int\s*\(\s*self\.root\.after\s*\("), "int(self.root.after(...))"),
    (re.compile(r"_after_jobs\s*\[\s*[^\]]+\s*\]\s*=\s*int\s*\("), "_after_jobs[...] = int(...)"),
]

ALLOW_LINE = re.compile(r"_normalize_after_job_id|Never parse|opaque string|delay_ms\)")
...
def main() -> int:
    ...
    for path in TARGETS:
        ...
        if "_after_jobs: dict[str, str]" not in text and path.name == "ui.py":
            errors.append(f"{path}: _after_jobs should store str tokens")
    ...
    if errors:
        print("verify_gui_after_job_ids FAILED")
        for err in errors:
            print(f"  [FAIL] {err}")
        return 1

    print("verify_gui_after_job_ids PASSED")
    return 0
