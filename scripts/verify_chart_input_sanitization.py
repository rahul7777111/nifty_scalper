...
def main() -> int:
    ...
    candles = [
        ...
        SimpleNamespace(time=now, open=100.5, high=102.0, low=100.0, close=101.0, volume=1100),  # duplicate ts
    ...
    try:
        ...
        for i in range(5):
            ...
            series.append(
                SimpleNamespace(time=t, open=100.0 + i, high=101.0 + i, low=99.0 + i, close=100.5 + i, volume=1000 + i)
            )
    ...
    if errors:
        print("verify_chart_input_sanitization FAILED")
        for err in errors:
            print(f"  - {err}")
        return 1

    print("verify_chart_input_sanitization PASSED")
    return 0
