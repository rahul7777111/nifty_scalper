...
class NiftyScalper:
    ...
    def _check_entry_liquidity(self, legs: List[Dict[str, object]]) -> tuple[bool, str]:
        ...
        if (not require_ba) and max_pct <= 0 and max_abs <= 0:
            return True, ""
    ...
    def _gpt_control_advise(
        ...
        *,
        ...
        try:
            ...
            model = "gpt-4o-mini"
        try:
            timeout_sec = float(getattr(self.cfg, "gpt_timeout_sec", 8.0) or 8.0)
        except Exception:
            timeout_sec = 8.0

        try:
            return gpt_advisor.advise_trade(
    ...
    def _gpt_entry_gate(self, *, proposal: Dict[str, Any], is_paper: bool) -> bool:
        ...
        if advice is None:
            if mode == "advice":
                print("[GPT] advisor unavailable (advice mode); allowing entry")
                return True
            print("[GPT GATE] advisor unavailable; blocking entry")
            return False
    ...
    def _gpt_auto_select_strategy(
        ...
        *,
            ...
            snap["chain_preview"] = [
                {
                    ...
                    "theta": self._estimate_option_theta_from_chain_row(r, float(spot)),
                }
        ...
        rec = ""
        try:
            rec = str(getattr(analysis, "recommended_strategy", "") or "").strip()
        except Exception:
            rec = ""
    ...
    def _open_directional_from_option(
        ...
        *,
        ...
        if append_state:
            try:
                max_pyr = int(getattr(self.cfg, "max_pyramid_levels", 0) or 0)
            except Exception:
                max_pyr = 0
            if max_pyr > 0:
    ...
    def _decide_entries(
        ...
        *,
        ...
        # If pyramiding is enabled, we allow entering 'directional' strategy 
        # as it will increment existing trade's quantity instead of creating a new position.
        if open_count >= self.cfg.max_open_positions:
            if int(getattr(self.cfg, "max_pyramid_levels", 0) or 0) <= 0 or (self.cfg.strategy_name or "").lower().strip() != "directional":
        ...
        if bool(getattr(self.cfg, "enable_roc_filter", False)) and roc_val is not None:
        ...
        if bool(getattr(self.cfg, "enable_choppiness_filter", False)) and chop_val is not None:
        ...
        if strat == "auto":
            ...
            # AUTO mode prefers GPT strategy selection; if GPT is unavailable or
            # returns a non-executable alias, fall back to deterministic heuristic
            # selection so all implemented branches remain reachable.
            try:
            ...
            if not gpt_rec:
                effective_strat = str(router.get("selected") or "directional")
                ...
                try:
                    gpt_reason = "GPT recommendation unavailable/invalid"
                    try:
                        if not bool(getattr(self.cfg, "gpt_enable", False)):
                            gpt_reason = "GPT auto-select disabled (set MSTOCK_GPT_ENABLE=1 to enable)"
                        elif not bool(getattr(self.cfg, "gpt_auto_select", True)):
                            gpt_reason = "GPT auto-select disabled (set MSTOCK_GPT_AUTO_SELECT=1 to enable)"
                        else:
                            api_key_present = bool(
                                str(os.getenv("MSTOCK_GPT_API_KEY") or os.getenv("OPENAI_API_KEY") or "").strip()
                            )
                            if not api_key_present:
                                gpt_reason = "GPT API key missing (set OPENAI_API_KEY or MSTOCK_GPT_API_KEY)"
                    except Exception:
                        pass

                    note = f"{effective_strat}|{gpt_reason}"
                    if note != str(getattr(self, "_last_auto_fallback_note", "") or ""):
                        print(f"[AUTO][FALLBACK] selected={effective_strat} ({gpt_reason})")
        ...
        if effective_strat in {
            ...
            if effective_strat in {
                ...
                if bool(getattr(self.cfg, "enable_trend_filter", True)) and trend_strength > float(ts_limit):
            ...
            if effective_strat in {
                ...
                rsi_low = float(sess_rsi_low)
                rsi_high = float(sess_rsi_high)
                if bool(getattr(self.cfg, "enable_premium_rsi_filter", True)):
        ...
        if effective_strat == "directional":
            ...
            if bool(getattr(self.cfg, "enable_choppiness_filter", False)):
                ...
                if spot is not None and entry_spot > 0:
                    if name in {"long_call", "short_put"}:
                        ...
                        if atr_val > 0:
                            ...
                            loss_mult = float(be_mult_local) if float(be_mult_local) < 0 else -abs_trail
                        ...
                        # Partial Booking Logic for Puts
                        if partial_mult > 0 and partial_pct > 0 and not bool(tr.get("partial_exit_done")):
                            partial_target_price = entry_spot - (partial_mult * atr_val)
                            try:
                                if tr.get("spot_target") is None:
